...
void pre_evolution (Array<double, 2> sitefreq, Array<int, 2> codon,Array<int, 2> seq_orig_sar,Array<int, 2> &seq_sar2,int *mutation_number_sar1,int m_loci_sar,int *mutation_location_sar,int seq_length, int day,int duplic,double selections)
{
    ...
    if (m_number_sar>0)
    {
        ...
        selection(m_number_sar, seq_sar2, mutation_temp,codon,selections);//selection
