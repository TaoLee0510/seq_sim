...
void random_mutation (Array<double, 2> sitefreq, Array<int, 2> codon,Array<int, 2> &seq_rand_sar,int m_loci_sar,int *mutation_location_sar,int seq_length,int j,int random_mutation_number,int duplic,int rand_times, double KaKS)
{
    Range all = Range::all();
    ...
    selection(random_mutation_number, seq_sar, mutation_temp,codon,KaKS);//selection
